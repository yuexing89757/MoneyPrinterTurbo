...
在 WebUI 顶部切换到 **批量生成**，每行输入一个关键词，即可一次提交最多 100 个独立视频任务。每个关键词都会自动生成一篇约 800 字的中文旁白文案，依次解释关键词的出处或形成背景、核心含义，以及它对现实生活的启发；随后自动完成配音、字幕、素材匹配和视频合成。

批量任务按输入顺序执行。页面会持续展示每项任务的阶段、进度、文案摘要和失败原因；已完成视频可以直接预览和下载。批次记录保存在 `storage/batches`，刷新页面或重启项目后仍可查看。重启时尚未完成的任务会显示为“已中断”，可在列表中单独重试；某个任务失败不会阻止后续关键词继续生成。
